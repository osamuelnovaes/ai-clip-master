...
# 2. Verificar se o venv foi movido e limpar
if [ -d "venv" ]; then
    # Se o venv estiver quebrado (apontando para pastas erradas), limpa
    if ! grep -q "$(pwd)" "venv/bin/activate"; then
        echo "🧹 Limpando ambiente antigo para evitar conflitos..."
        rm -rf venv
    fi
fi
